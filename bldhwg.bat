...
%HRB_DIR%\bin\harbour %SRC_PATH%\sounds.prg %SRC_PATH%\htrack.prg %SRC_PATH%\hmessage.prg %SRC_PATH%\mscz.prg %SRC_PATH%\mxl.prg %SRC_PATH%\midi.prg -n -q -w -i%HRB_DIR%\include;%HWGUI_DIR%\include 2>hberr.log

echo 1 24 "c:\papps\hwgui_218\image\WindowsXP.Manifest" > hwgui_xp.rc
brc32 -r hwgui_xp -fohwgui_xp

bcc32 -c -O2 -tW -M -I%HRB_DIR%\include;%HWGUI_DIR%\include;%PORTAUDIO_DIR%;%SNDFILE_DIR%\include sounds.c htrack.c hmessage.c midi.c mscz.c mxl.c hcediext.c %SRC_PATH%\hb_pa.c
ilink32 -Gn -aa -Tpe -L%HRB_DIR%\lib\win\bcc;%HWGUI_DIR%\lib c0w32.obj sounds.obj htrack.obj hmessage.obj midi.obj mscz.obj mxl.obj hcediext.obj hb_pa.obj, sounds.exe, sounds.map, %HWGUI_LIBS% %HRB_LIBS% ws2_32.lib cw32.lib import32.lib libportaudio.lib libsndfile.lib,, hwgui_xp.res
